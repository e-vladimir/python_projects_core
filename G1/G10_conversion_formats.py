...
def DTimeToString(dtime: datetime.datetime) -> str:
...
def IntegersToStrings(data: list[int]) -> list[str]:
...
def StringToDateTime(text: str) -> datetime.datetime | None:
	...
	raw       = raw.replace( ',',  '')
	raw       = raw.replace('  ', ' ')
	raw       = raw.replace('янв', '01')
	raw       = raw.replace('фев', '02')
	raw       = raw.replace('мар', '03')
	raw       = raw.replace('апр', '04')
	raw       = raw.replace('май', '05')
	raw       = raw.replace('июн', '06')
	raw       = raw.replace('июл', '07')
	raw       = raw.replace('авг', '08')
	raw       = raw.replace('сен', '09')
	raw       = raw.replace('окт', '10')
	raw       = raw.replace('ноя', '11')
	raw       = raw.replace('дек', '12')
...
def StringsToBooleans(strings: list[str]) -> list[float]:
	""" Преобразование строк в логические значения """
	return [StringToBoolean(item) for item in strings]


def StringsToDTimes(texts: list[str]) -> list[datetime.datetime]:
	""" Преобразование строк в формат даты/времени """
	return [StringToDateTime(item) for item in texts]


def StringsToFloats(texts: list[str]) -> list[float]:
	""" Преобразование строк в дробные числа """
	return [StringToFloat(item) for item in texts]


def StringsToIntegers(texts: list[str]) -> list[int]:
	""" Преобразование строк в целые числа """
	return [StringToInteger(item) for item in texts]
